...
def get_count(url: str) -> int | None:
    try:
        ...
        # Method 2: span with "count" in class inside the header (search pages)
        # Targets b-search-results__header-heading-count which contains "(1)"
        # Avoids the heading span which contains "jordan 4 retro"
        count_el = soup.select_one(".b-search-results__header [class*='count']")
        if count_el:
            text = count_el.get_text(strip=True)  # e.g. "(1)"
            match = re.search(r"\d+", text)
            if match:
                return int(match.group())

        # Method 3: hidden input data-products-count
